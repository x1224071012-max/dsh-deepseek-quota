...
`;
		...
		async function requestQuota(scope) {
			const url = scope === "balance" ? `${ROUTE}?scope=balance` : ROUTE;
		...
		function QuotaView() {
			...
			const head = React.createElement(
				...
				React.createElement(
					"button",
					{ className: "dsq-btn", type: "button", disabled: loading, onClick: quota.refresh },
					loading ? "读取中…" : "刷新",
				),
			);
		...
		function Chip() {
			...
			return React.createElement(
				"button",
				{ className: "dsq-chip", type: "button", title, onClick: quota.refresh },
